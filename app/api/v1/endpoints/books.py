...
async def update_book_handler(
    ...
    old_status = book_obj.status if book_obj else None  # capture BEFORE any mutation
    if book_obj:
        if direction:
    ...
    _warning_keys = ['violence','language','dark_themes','drugs','self_harm','main_death','bad_ending','tragedy','betrayal','noncon']
    _active_w = [k for k in _warning_keys if form_data.get('warning_' + k)]
    import json as _j3
    if book_obj:
        book_obj.content_warnings = _j3.dumps(_active_w) if _active_w else None
        await db.flush()
    ...
    if tags_json is not None:
        try:
            ...
            pass
    # Update content warnings (form_data already fetched above)
    if book_obj:
        warnings = [k[8:] for k in form_data.keys() if k.startswith('warning_')]
    ...
    if book_obj and status_enum != old_status:
        from app.services.book_service import notify_book_subscribers, STATUS_LABELS_RU
        status_label = STATUS_LABELS_RU.get(status_enum.value, status_enum.value)
        await notify_book_subscribers(
            db,
            book_obj,
            kind="book_status",
            title=f'Книга «{book_obj.title}» изменила статус',
            body=f'Новый статус: {status_label}',
...
async def create_new_chapter(
    ...
    if is_published:
        ...
        await notify_book_subscribers(
            ...
            body=f'Опубликована глава {chapter.number}: «{chapter.title}»',
...
async def update_chapter_handler(
    ...
    # Notify subscribers if the chapter was just published for the first time
